...
export default class Side extends Component<Props> {
    ...
    componentDidMount() {
        this.emitter = DeviceEventEmitter.addListener("remain", (data) => {
            let t = this;
    ...
    findcommunity = () => {
        let t = this;
        if (this.state.headcontent != '') {
            let value = {
                uid: this.state.uid,
                text: this.state.headcontent
            }
            let t = this;
            http.post('http://192.168.137.1:8763/findCommunityByText', value)
                .then(function (data) {

                    if (data[0].flag == 'Found') {
        ...
        const menu = <View>
            ...
            <LinearGradient colors={['#B0E0E6', '#6495ED']} >
                <View style={styles.container4}>
                    <TouchableOpacity onPress={() => { t.props.navigation.navigate('Favorite', { message: 'Favorite' }) }}>
                        <Text style={{ marginLeft: 10, fontSize: 20, marginTop: 30, color: '#000000' }}>我的收藏</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => { t.props.navigation.navigate('MyMain', { message: 'MyMain' }) }}>
